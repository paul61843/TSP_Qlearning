...
def run_n_NJNP_Comp(
    ...
    if current_time % env.record_time == 0:
        ...
        csv_utils.writeDataToCSV(f'./result/csv/csv{process_index}/NJNP-Comp.csv', env.result)

    # 產生UAV路徑圖
    if current_time % 1000 == 0:
        uav_run_img = env.render(return_img = True)
        imageio.mimsave(f"./result/NJNP_Comp/{current_time}_time_index{process_index}_UAV_result.gif",[uav_run_img],fps = 10)
...
def run_n_subTree(
    ...
    if len(env.stops) == 1:
        env.unvisited_stops = env.get_unvisited_stops()
        a = getHeightSubTreeNextPoint(env, child_nodes)
        env.stops.append(a)
    ...
    if current_time % 1000 == 0:
        uav_run_img = env.render(return_img = True)
        imageio.mimsave(f"./result/subTree_and_mutihop/{current_time}_time_index{process_index}_UAV_result.gif",[uav_run_img],fps = 10)
